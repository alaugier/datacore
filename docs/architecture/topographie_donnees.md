...
| **S3** (Simple Storage Service) | Service de stockage objet d'AWS ; son API HTTP est devenue un standard de fait, repris par des solutions non-AWS comme **MinIO** (bloc 4) pour rester compatibles avec les mêmes outils sans dépendre d'AWS. |
| **Bucket** | Conteneur de premier niveau d'un stockage objet S3/MinIO (grossièrement, un répertoire racine). OMEGA LAKE utilise un **bucket unique**, `omega-lake` (variable `OMEGA_LAKE_BUCKET`) — les zones `raw`/`staging`/`curated` sont des préfixes à l'intérieur de ce bucket, pas des buckets séparés. |
| **Zone** (data lake) | Une étape de transformation à l'intérieur du bucket unique (`raw`/`staging`/`curated`) — un préfixe de chemin, pas un bucket séparé. Voir [`organisation_omega_lake.md`](organisation_omega_lake.md) pour la hiérarchie complète bucket → zone → flux → objet. |
| **Flux** (data lake) | Une source de données IoT distincte, avec son propre schéma (ex. `capteurs_temperature`, `rfid_scans`) — un sous-préfixe à l'intérieur de chaque zone. 5 flux au total, voir [`organisation_omega_lake.md` §3](organisation_omega_lake.md#3-les-5-flux-en-un-coup-dœil). |
| **s3://** | Convention d'URI (`s3://<bucket>/<chemin>`) pour désigner un objet dans un stockage compatible S3 — traduite en requêtes HTTP(S) par l'outil qui la lit (ex. l'extension `httpfs` de DuckDB). |
| **httpfs** | Extension DuckDB qui sait lire/écrire des fichiers via HTTP(S) ou `s3://`, y compris contre un stockage S3-compatible non-AWS comme MinIO — nécessite alors une configuration explicite d'endpoint (`s3_endpoint`) et de style d'adressage (`s3_url_style='path'`), MinIO n'étant pas AWS S3. Voir [`integration_infrastructure_omega_lake.md` §1.3bis](integration_infrastructure_omega_lake.md#13bis-même-preuve-cette-fois-avec-le-parquet-réellement-dans-minio). |
...
| **Diagramme de Gantt** | Représentation graphique d'un planning : une barre horizontale par tâche, positionnée dans le temps, montrant enchaînements et chevauchements entre phases. Utilisé pour la planification du programme — voir [`feuille_de_route.md` §4.1](feuille_de_route.md#41-diagramme-de-gantt), maintenu en Mermaid (texte versionné dans le dépôt) plutôt que dans un outil graphique externe. |
| **BI** (Business Intelligence) | Ensemble des pratiques et outils visant l'**aide à la prise de décision** à partir de données consolidées (tableaux de bord, indicateurs, restitution visuelle) — par opposition à la donnée brute ou à son seul stockage. Chez Omega Logistics : l'entrepôt **OMEGA BI** (bloc 3) et sa restitution dans **Grafana** (C16bis) — voir [`gestion_operationnelle_omega_bi.md` §3](gestion_operationnelle_omega_bi.md#3-tableau-de-bord-des-indicateurs-de-service). |
